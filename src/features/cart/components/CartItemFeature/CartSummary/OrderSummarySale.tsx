...
import { calculateDiscount } from "@/lib/utils/calculate-discount";
...
export default function CartSummary({ subtotal }: { subtotal: number }) {
    ...
    const discount = applied ? calculateDiscount(subtotal, applied.value, applied.type) : 0;
        ...
        <div className="p-6 mb-5 bg-white rounded-xl border space-y-4">
            ...
            {applied && (
                <div className="flex justify-between text-red-500 md:text-[20px]">
                    <span className="text-[#666666]">Discount ({applied.type === "percent" ? `-${applied.value}%` : `-$${applied.value}`})</span>
