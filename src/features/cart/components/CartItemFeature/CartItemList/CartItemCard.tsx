            ...
            <div key={cartItem.id} className="flex w-full relative">
                ...
                <div className="flex-1 flex flex-col justify-between">
                    <div className="flex flex-col">
                        <h4 className="text-md font-semibold md:text-lg">{cartItem.productName}</h4>
                        <div className="text-xs md:text-md text-gray-600">
                            <p><strong>Size:</strong> {cartItem.variant.size}</p>
                            <p><strong>Color:</strong> {cartItem.variant.colorName}</p>
                        </div>
                    </div>
                    <div className="mt-2 flex items-center justify-between">
                        <h2 className="text-xl font-bold md:text-2xl">${cartItem.price}</h2>
                        <QuantityPicker className="w-1/2 h-[30px] md:w-1/5 md:h-[40px]" value={cartItem.quantity} />
                    </div>
                </div>
                <span className="absolute top-1 right-1"><TrashIconSVG /></span>
            </div>
        </div>
    );
}
