...
export default function ProductDetailVariant({ product }: { product: Product }) {
    const [variantIndex, setVariantIndex] = useState(0);
    const [selectedSize, setSelectedSize] = useState<string | null>(null);
    const variant = product.variants[variantIndex];
    return (
        <div className="md:flex-col md:justify-between md:h-[520px]">
            <h2 className="md:text-[40px] text-2xl md:pt-0 pt-5 font-display pb-5 font-bold">{product.name}</h2>
            <div className="flex"><StarRating rating={product.rating} /><span className="ml-2">{product.rating}/5</span></div>
            <div className="flex pt-5 ">
                <h3 className="text-2xl md:text-[32px] font-bold mr-2">${product.price}</h3>
                {product.oldPrice && (
            ...
            <div className="md:items-end mx-auto">
                    ...
                    {product.variants.map((v: Variant, idx: number) => (
                        <button
                            ...
                            className={`w-9 h-9 rounded-full border-2 flex items-center justify-center relative transition
                                ${idx === variantIndex ? '' : 'border-gray-300'}
                                                             `}
                    ...
                    <div className="flex gap-3 mt-3 overflow-x-auto scrollbar-hide">
                        {variant.sizes.map((s: any) => (
                            <button
                                ...
                                className={`px-4 md:h-[50px] py-2 rounded-full border text-sm ${selectedSize === s.label
                ...
                <div className="flex pt-5 gap-3">
                    <QuantityPicker min={1} max={10} className="w-1/3 md:h-[50px] rounded-3xl h-10" />
                    <Button className="w-3/5 rounded-3xl h-10 md:h-[50px] cursor-pointer flex-end">Add to Cart</Button>
                </div>
            </div>
        </div>
